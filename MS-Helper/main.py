...
from modules.fitting import peaks_finder_callback, draw_fitted_peaks, rolling_window_fit
...
with dpg.window(label="Control", width=1430, height=-1, no_close=True, no_collapse=True, no_move=True, no_resize=True, no_title_bar=True):
...
# Create a window
with dpg.window(label="Data Filtering and peak finding",width=1430, pos=(0,60), no_close=True, no_move=True, no_resize=True, tag="Data Filtering", collapsed=False):
    ...
    with dpg.group(horizontal=True, horizontal_spacing= 50):
        ...
        dpg.add_slider_int(label="Baseline window", default_value=500, min_value=10, max_value=1000, callback=spectrum.request_baseline_update, user_data=spectrum, tag="baseline_window")
    ...
    with dpg.group(horizontal=True, horizontal_spacing= 50):
        dpg.add_slider_int(label="Peak detection distance from baseline", width=200, default_value=100, min_value=1, max_value=300, tag="peak_detection_threshold")
        dpg.add_slider_int(label="Peak width", width=200, default_value=20, min_value=2, max_value=100, tag="peak_detection_width")
        dpg.add_slider_int(label="Peak min distance", width=200, default_value=100, min_value=1, max_value=1000, tag="peak_detection_distance")
        #dpg.add_slider_int(label="Max peak width", width=200, default_value=40, min_value=2, max_value=100, tag="max_peak_width")
    dpg.add_button(label="Find Peaks", callback=peaks_finder_callback, user_data=spectrum)


with dpg.window(label="Peak fitting", width=1430, height=800, pos=(0,85), no_close=True, no_move=True, no_resize=True, tag="Peak fitting"):
...
with dpg.window(label="Peak matching", width=1430, height=800, pos=(0,110), no_close=True, no_move=True, no_resize=True, tag="Peak matching"):
    # Create a plot for the raw data
